#!/usr/bin/env python
"""v2 at T=0.3 — 250-doc gate run for ensemble diversity.

Uses the first 250 docids of v2_preds.json (tools/val_250_docids.json).
Resumes from predictions/v2_t03_250.json if it exists (100 docs pre-seeded
from partial 500-doc run).

v2 baseline (T=1.0): KILE evaluated on same 250 docs.

Usage:
    DATA_ROOT=data uv run python tools/v2_t03_250.py
"""
...
OUT_PATH = PROJECT_ROOT / "predictions" / "v2_t03_250.json"
V2_250_BASELINE = PROJECT_ROOT / "predictions" / "v2_t00_250.json"
...
async def run_batch() -> None:
    ...
    print(f"Flags: BD_USE_REFINER=0 BD_TEMPERATURE=0.3 BD_ALT_PROMPT=0")
    ...
    print(f"\nv2_t03_250: KILE {kile_ap:.2f}% / LIR {lir_f1:.2f}%")
