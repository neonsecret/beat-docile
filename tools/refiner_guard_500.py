#!/usr/bin/env python
"""500-doc refiner guard mode run.

BD_USE_REFINER_GUARD=1: refiner only removes known label words; spatial tightening
is blocked unless it only strips labels. Line items exempt (they need row isolation).
Trailing currency fix also active (in _refine_amount).

50-doc result: KILE +3.01pp (41.86→44.87%), LIR -0.28pp (52.36→52.08%) vs V5b-50.
Beats v2-500 baseline (44.61% KILE / 50.89% LIR) on KILE.

Usage:
    DATA_ROOT=data uv run python tools/refiner_guard_500.py
"""
...
OUT_PATH = PROJECT_ROOT / "predictions" / "refiner_guard_500.json"
...
async def run_batch() -> None:
    ...
    print(f"Flags: BD_USE_REFINER=1 BD_USE_VALIDATOR=1 BD_USE_REFINER_GUARD=1 BD_USE_BBOX_VERIFY=0")
    print(f"Target: beat v2-500 ({V2_500_KILE}% KILE / {V2_500_LIR}% LIR)")
    ...
    print(f"Guard mode (500):    KILE {kile_ap:.2f}% / LIR {lir_f1:.2f}%")
    ...
    if kile_ap > V2_500_KILE and lir_f1 > V2_500_LIR:
        print(f"\n✅ BEATS v2 on BOTH metrics — guard mode is the new baseline")
    elif kile_ap > V2_500_KILE:
        print(f"\n⚠️  KILE beats v2 (+{kile_ap - V2_500_KILE:.2f}pp), LIR behind by {lir_f1 - V2_500_LIR:.2f}pp")
    else:
        print(f"\n❌ Does not beat v2-500 — investigate before committing")
