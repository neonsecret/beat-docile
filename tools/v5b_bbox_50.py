#!/usr/bin/env python
"""A/B eval: BD_USE_BBOX_VERIFY=1 vs V5b baseline on same 50 val docs.

Sets BD_USE_BBOX_VERIFY=1 BD_USE_REFINER=1 BD_USE_VALIDATOR=1.
Monkey-patches verify_bbox to count corrected=True triggers.
Saves to predictions/v5b_bbox_50.json, then evals KILE AP / LIR F1.

Usage:
    DATA_ROOT=data uv run python tools/v5b_bbox_50.py
"""
...
OUT_PATH = PROJECT_ROOT / "predictions" / "v5b_bbox_50.json"
...
async def run_batch() -> None:
    ...
    print(f"BD_USE_BBOX_VERIFY=1 BD_USE_REFINER=1 BD_USE_VALIDATOR=1")

    # Load the same 50 docids as v5b baseline
    ...
    print(f"V5b + bbox_verify:     KILE AP {kile_ap:.2f}% / LIR F1 {lir_f1:.2f}%")
