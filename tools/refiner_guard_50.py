#!/usr/bin/env python
"""A/B test: BD_USE_REFINER_GUARD=1 on same 50 val docs as v5b_50 baseline.

Guard mode: if refiner would remove any non-label word, fall back to
just stripping the label prefix. Prevents over-aggressive spatial tightening
of addresses, names, and amounts.

Also tests trailing-currency fix in _refine_amount.

Compare to v5b_50 baseline (41.86% KILE / 52.36% LIR).

Usage:
    DATA_ROOT=data uv run python tools/refiner_guard_50.py
"""
...
OUT_PATH = PROJECT_ROOT / "predictions" / "refiner_guard_50.json"
...
async def run_batch() -> None:
    ...
    print(f"Refiner guard (50): KILE {kile_ap:.2f}% / LIR {lir_f1:.2f}%")
    print(f"Delta:              KILE {kile_ap - V5B_KILE:+.2f}pp / LIR {lir_f1 - V5B_LIR:+.2f}pp")

    if kile_ap > V5B_KILE and lir_f1 > V5B_LIR:
        print(f"\n✅ BOTH metrics improved over V5b-50 — scale to 500-doc")
    elif kile_ap > V5B_KILE:
        print(f"\n⚠️  KILE improved but LIR regressed — investigate per-field")
    elif kile_ap > V2_KILE:
        print(f"\n⚠️  Beats v2 on KILE — check LIR before committing")
    else:
        print(f"\n❌ KILE did not improve — investigate guard trigger rate")
